...
timeline caps its lookback. Dates are inclusive UTC calendar days: `days` counts whole days ending
today, `from` on its own ends at today, and `to` on its own covers the seven days ending at `to`. A
`to` in the future is rejected, and `days` cannot be combined with `from`/`to`:
...
`limit` (1 to 100, default 20) is advisory. A response covers whole calendar days: results are
rounded down to a day boundary, and the newest day of the window comes back in full even when it
holds more events than `limit`.
