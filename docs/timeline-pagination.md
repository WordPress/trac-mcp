...
The coverage contract applies when a request uses `from`, `to`, or `author`. A request that uses only `days` and `limit` keeps the original recent-timeline contract: Trac receives `max=limit`, and the response contains `results`, `totalEvents`, `daysBack`, and `timelineUrl`.

- A request names a window, as `days` or as `from`/`to`. The response echoes it as `requested`.
- `covered` is the sub-window the response covers completely. It contains only whole days, or it is `null` when the response cannot cover any complete day.
- `complete` is true when `covered` equals `requested` and every day in it was seen whole. When it is false, `continueWith` holds the `from`/`to` of a whole-day uncovered remainder together with the effective `limit` and accepted `author` input, so it can be submitted unchanged when present.
...
One case this does not serve is a single day holding more events than the fetch cap. The response then cannot claim to have seen that day whole: `covered` is `null`, `complete` is false, and `terminalTruncation` names the day, the fetch limit, the returned event count, and that the tool cannot continue within that day. `note` gives the same warning in words. A `continueWith` can still cover older days in the requested window, but it cannot recover the missing events from the truncated day. Splitting inside a day would reintroduce the intra-day position the model exists to avoid, so the case is reported rather than engineered around. core.trac runs at tens of events per day.
...
- Callers that only want the newest activity do not have to know any of this: a bare call still uses the original response contract and answers for the last seven days in one request.
